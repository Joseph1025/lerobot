...
def load_hdf5_episode(file_path: Path) -> Dict[str, np.ndarray]:
    ...
    with h5py.File(file_path, 'r') as f:
        ...
        for frame_idx in range(episode_length):
            ...
            data['observation.images.left_wrist'][frame_idx] = decompress_jpeg_image(
                images_group['left_wrist'][frame_idx]
            )
            data['observation.images.right_wrist'][frame_idx] = decompress_jpeg_image(
                images_group['right_wrist'][frame_idx]
            )
            data['observation.images.top'][frame_idx] = decompress_jpeg_image(
                images_group['top'][frame_idx]
            )
